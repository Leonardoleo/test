...
let TokenManager = class TokenManager {
    ...
    refresh([auth, ...restAliases] = []) {
        ...
        return this.obtainTokens(auth, this.refreshUrl, { refresh_token: tokens.refresh_token }).pipe(tap((refreshedTokens) => [auth, ...restAliases].forEach((alias) => this.storage.setTokens(alias, refreshedTokens))), catchError(() => {
    ...
    obtainTokens(auth, url, body) {
        ...
        }).pipe(tap((refreshedTokens) => resolve(refreshedTokens.access_token)), catchError((error) => {
    ...
    getErrorMessage({ httpStatus: { code, status } }) {
        if (code === 404) {
